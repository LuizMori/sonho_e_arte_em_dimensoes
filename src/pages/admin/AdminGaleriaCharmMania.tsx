...
import { uploadCharmManiaGalleryImage } from "@/lib/storage";
import type { CharmManiaGalleryItem } from "@/types";

export function AdminGaleriaCharmMania() {
  usePageMeta(
    "Galeria Charm Mania | Admin | Sonho e Arte em Dimensões",
    "Publique fotos de peças Charm Mania já montadas."
  );

  const { showToast } = useToast();
  const [itens, setItens] = useState<CharmManiaGalleryItem[]>([]);
  ...
  const carregar = async () => {
    ...
    setItens((data as CharmManiaGalleryItem[]) ?? []);
    setCarregando(false);
  };
  ...
  const publicarFoto = async () => {
    ...
    setEnviando(true);
    try {
      const url = await uploadCharmManiaGalleryImage(arquivo);
      const { error } = await supabase.from("charm_mania_gallery").insert({
        imagem_url: url,
  ...
  const remover = async (id: string) => {
    ...
    const { error } = await supabase.from("charm_mania_gallery").delete().eq("id", id);
    if (error) {
      ...
      <div className="container max-w-3xl">
        <Reveal className="mb-12">
          ...
          <p className="text-sm text-navy/50 mt-4">
            Fotos de peças Charm Mania já montadas, mostradas na aba Charm Mania do Portfólio.
          </p>
        </Reveal>
        ...
        <Reveal className="border border-neutral-light rounded-xl px-6 py-6 mb-16">
          ...
          <div className="flex flex-wrap items-end gap-4">
            <div className="flex-1 min-w-[200px]">
              <Label htmlFor="descricaoFotoCharmMania">Descrição (opcional)</Label>
              <Input
                id="descricaoFotoCharmMania"
                value={descricao}
                onChange={(e) => setDescricao(e.target.value)}
                placeholder="Ex: Chaveiro com nome e coração"
              />
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {itens.map((item) => (
              <div key={item.id} className="group relative aspect-square overflow-hidden rounded-xl">
                <img
                  src={item.imagem_url}
                  alt={item.descricao ?? "Peça Charm Mania"}
