...
export async function uploadCharmImage(file: File): Promise<string> {
  const extensao = file.name.split(".").pop() ?? "jpg";
  const caminho = `charms/${crypto.randomUUID()}.${extensao}`;
...
export async function uploadCharmManiaGalleryImage(file: File): Promise<string> {
  const extensao = file.name.split(".").pop() ?? "jpg";
  const caminho = `charm-mania-gallery/${crypto.randomUUID()}.${extensao}`;
