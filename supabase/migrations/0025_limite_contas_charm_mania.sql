-- Limite de contas (letras + pingentes) por peça Charm Mania — não é um valor fixo no
-- código porque cordões diferentes (hoje ou no futuro) podem ter tamanhos diferentes;
-- fica editável junto do produto, como valor_letra. Null = sem limite.

alter table public.products add column limite_contas integer check (limite_contas is null or limite_contas > 0);

-- Reescreve criar_pedido (mesmo corpo de 0023_charm_mania.sql) só para validar, antes de
-- montar o item, que a sequência enviada não ultrapassa o limite_contas do produto-base —
-- nunca confiando no client, mesmo que o montador já bloqueie isso na interface.
