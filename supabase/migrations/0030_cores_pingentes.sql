-- Cor por pingente: mesmo mecanismo já usado para cordão/caixinha (product_colors) e letras
-- (charm_mania_letter_colors), mas pingentes não são produtos — precisam de tabela própria
-- charm_colors ligando charms à paleta compartilhada de colors. A cliente escolhe, ao
-- adicionar um pingente à peça, uma das cores cadastradas para aquele pingente (quando
-- houver); a cor escolhida vai junto no snapshot do pedido, mesmo padrão da cor da letra.
--
-- Também remove a menção a "ponteira" da peça-base, que deixou de existir como conceito
-- separado (a peça-base é só o cordão).

create table public.charm_colors (
  id uuid primary key default gen_random_uuid(),
  charm_id uuid not null references public.charms(id) on delete cascade,
  color_id uuid not null references public.colors(id) on delete cascade,
  created_at timestamptz not null default now(),
  unique (charm_id, color_id)
);

create index on public.charm_colors(charm_id);

alter table public.charm_colors enable row level security;
create policy "charm_colors_select_all" on public.charm_colors for select using (true);
create policy "charm_colors_insert_admin" on public.charm_colors for insert with check (public.is_admin());
create policy "charm_colors_delete_admin" on public.charm_colors for delete using (public.is_admin());

-- Reescreve criar_pedido (mesmo corpo de 0027_cores_charm_mania.sql) só para propagar a cor
-- de cada pingente (v_conta->>'cor') para a versão resolvida da conta gravada no snapshot.
create or replace function public.criar_pedido(
  p_user_id uuid,
  p_itens jsonb, -- [{"product_id": "...", "quantidade": 1, "cor": null, "variacao": null, "personalizacao": {"sequencia": [{"tipo":"letra","valor":"A","cor":"Azul"}, {"tipo":"pingente","charmId":"...","cor":"Rosa claro"}, ...], "cordaoCor": "Rosa", "caixinha": true, "caixinhaCor": "Azul claro"} | null}, ...]
  p_cep_destino text,
    ...
    if v_item.personalizacao is not null then
      ...
      loop
        if v_conta->>'tipo' = 'letra' then
          v_letras := v_letras + 1;
          v_cor_letra := v_conta->>'cor';
          if v_cor_letra is not null and not (v_cor_letra = any(v_cores_letra)) then
            v_cores_letra := array_append(v_cores_letra, v_cor_letra);
          end if;
      ...
      if array_length(v_cores_letra, 1) > 2 then
        raise exception 'A peça pode usar no máximo 2 cores de letra';
      end if;
  ...
  loop
      ...
      loop
        if v_conta->>'tipo' = 'letra' then
          ...
          v_sequencia_final := v_sequencia_final || jsonb_build_array(
            jsonb_build_object(
              'tipo', 'pingente', 'charmId', v_charm_id, 'nome', v_charm_nome, 'preco', v_charm_preco,
              'cor', v_conta->>'cor'
            )
          );
    ...
    values (
      ...
      case when v_item.personalizacao is null then null else jsonb_build_object(
        ...
        'cordao_cor', v_item.personalizacao->>'cordaoCor',
    ...
    -- Reserva do produto-base (cordão da Charm Mania, ou produto normal).
