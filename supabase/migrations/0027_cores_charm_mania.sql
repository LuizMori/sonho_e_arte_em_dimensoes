-- Cor do cordão: reaproveita o mecanismo já existente de product_colors (o mesmo usado
-- por qualquer produto) — o produto-base da Charm Mania já pode ter "Cores disponíveis"
-- atribuídas normalmente pelo formulário de produto, sem precisar de tabela nova.
--
-- Cor das letras é outra lista, porque é um conjunto de opções diferente do cordão (nem
-- toda cor de cordão existe em filamento de letra, e vice-versa): tabela própria
-- charm_mania_letter_colors, mesmo padrão de product_colors. A cliente pode usar no máximo
-- 2 cores distintas entre as letras da peça — validado aqui no banco, nunca só no client.

create table public.charm_mania_letter_colors (
...
create index on public.charm_mania_letter_colors(product_id);

alter table public.charm_mania_letter_colors enable row level security;
create policy "charm_mania_letter_colors_select_all" on public.charm_mania_letter_colors for select using (true);
create policy "charm_mania_letter_colors_insert_admin" on public.charm_mania_letter_colors for insert with check (public.is_admin());
create policy "charm_mania_letter_colors_delete_admin" on public.charm_mania_letter_colors for delete using (public.is_admin());

-- Reescreve criar_pedido (mesmo corpo de 0026_cor_caixinha_charm_mania.sql) para: (1)
-- gravar a cor do cordão escolhida no snapshot, e (2) validar que a sequência não usa mais
-- de 2 cores de letra distintas.
create or replace function public.criar_pedido(
  p_user_id uuid,
  p_itens jsonb, -- [{"product_id": "...", "quantidade": 1, "cor": null, "variacao": null, "personalizacao": {"sequencia": [{"tipo":"letra","valor":"A","cor":"Azul"}, ...], "cordaoCor": "Rosa", "caixinha": true, "caixinhaCor": "Azul claro"} | null}, ...]
  p_cep_destino text,
